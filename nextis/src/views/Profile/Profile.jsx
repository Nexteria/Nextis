...
import ProfileAvatarForm from 'views/Profile/ProfileAvatarForm';
...
class Profile extends React.Component {
    ...
    componentWillReceiveProps(nextProps) {
        const { isProfileInitialized } = this.state;
        if (!isProfileInitialized && nextProps.data.user) {
            const stateChange = {
    ...
    async updateProfile() {
        ...
        const values = {};
        formItems.forEach((item) => {
            const { [item.id]: formValue } = this.state;
        ...
        if (isFormValid) {
            ...
            updateProfile({ variables: values }).then((updateResponse) => {
                ...
                data.refetch();
                allSkillsData.refetch();

                setSelectedSkills(() => updateResponse.data.UpdateUserProfile.skills);
            }).catch((errorData) => {
