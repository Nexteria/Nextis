    ...
    payload: {
      promise: fetch('/users', {
        ...
        body: JSON.stringify({
          ...
          dateOfBirth: values.dateOfBirth ? format(values.dateOfBirth, 'YYYY-MM-DD HH:mm:ss') : null,
          hobby: values.hobby.toString('html'),
          otherActivities: values.otherActivities.toString('html'),
        }),
      }).then(response => response.json()),
    },
  });
